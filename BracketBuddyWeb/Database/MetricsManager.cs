...
public class MetricsManager
{
    ...
    private List<MetricsModel> _metricsModels;

    public MetricsModel CurrentModel => _metricsModels.Last();
    ...
    private MetricsManager()
    {
        ...
        _metricsModels = new List<MetricsModel>() { new MetricsModel() };
    ...
    private async void FireMetricsAndDoCleanup(object sender, ElapsedEventArgs e)
    {
        lock (_lock)
        {
            _metricsModels.Add(new MetricsModel());
        }

        var oldModel = _metricsModels.First();
        oldModel.EndTime = DateTimeOffset.UtcNow;
        await BracketBuddyDatabase.Instance.UpsertMetricsAsync(oldModel);
